...
//int uartWriteBuf(unsigned char *buf, size_t len)
int LobotSerialWrite(unsigned char *buf, size_t len)
{

    printf("%s%s%d\n", __func__,__FILE__, __LINE__);
    uart_write(fd, (const char*)buf, len);

}

//int uartReadBuf(unsigned char *buf, size_t len)
int LobotSerialRead(unsigned char *buf, size_t len)
{

    uart_read(fd, (char*)buf, len);
