...
void LobotSerialMotorMove(uint8_t id, int16_t speed)
{
  printf("%s%s%d\n", __func__,__FILE__,__LINE__);
  uint8_t buf[10];
  if (speed > 1000)
  ...
  buf[5] = 1;
  buf[6] = 0;
  buf[7] = GET_LOW_BYTE(uspeed);
  buf[8] = GET_HIGH_BYTE(uspeed);
  buf[9] = LobotCheckSum(buf);
  LobotSerialWrite(buf, 10);
  printf("%s%s%d\n", __func__,__FILE__,__LINE__);
}
...
void LobotSerialServoStop(uint8_t id)
{
  printf("%s%s%d\n", __func__,__FILE__,__LINE__);
  uint8_t buf[6];
  buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
...
void LobotSerialServoStart(uint8_t id)
{
  printf("%s%s%d\n", __func__,__FILE__,__LINE__);
  uint8_t buf[6];
  buf[0] = buf[1] = LOBOT_SERVO_FRAME_HEADER;
